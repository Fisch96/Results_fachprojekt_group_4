...
def get_incomplete_sample_nrs(df):
    ...
    for ilp_method in df['algorithm_complete'].dropna().unique():
...
def prepare_data_and_plot(df, title, plot_type):
    ...
    #################################################################################################################################################
    df["algorithm_complete"] = df["algorithm_complete"].str.replace("OdemandFirstWaypoints", "ODemandFirstWaypoints", case=False , regex=False)
    df["algorithm_complete"] = df["algorithm_complete"].str.replace("TLInverseCapacity", "TLInverse", case=False, regex=False)
