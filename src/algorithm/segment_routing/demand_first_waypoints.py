...
class DemandsFirstWaypoints(GenericSR):
    ...
    def __init__(self, nodes: list, links: list, demands: list, weights: dict = None, waypoints: dict = None, **kwargs):
        ...
        # initial weights # Modified: initialize weights based on inverse capacity
        self.__weights = weights if weights else self.__init_inverse_capacity_weights()
    ...
    #Verbindungsgewichte basierend auf inverser Kapazität zuweisen
    def __init_inverse_capacity_weights(self):
        max_capacity = max(self.__capacities.values())
        return {(u, v): max_capacity / self.__capacities[(u, v)] for u, v in self.__links}
        ...
        for s in range(self.__n):
            ...
            for t in range(self.__n):
                ...
                for u_idx in range(self.__n - 1):
                 ...
                 ##Hier Devision durch Null vermeiden 
                    if len(successors) == 0:
                     new_fraction = 0
                    else:
